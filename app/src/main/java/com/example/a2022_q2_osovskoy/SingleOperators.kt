...
object SingleOperators {
    ...
    object Task3 {
         ...
         */
        fun solve(provider: DataProvider): Single<Int> = Single.create { emitter ->
            emitter.onSuccess(provider.get())
        }
    ...
    object Task4 {
         ...
         */
        fun solve(source: Observable<Int>): Observable<Int> = source.map { num -> num - 1 }
    }
    ...
    object Task5 {
        ...
        fun solve(name: String, api: UserApi): Single<User> = api.create(name).map { userId ->
            User(name, userId)
        }
